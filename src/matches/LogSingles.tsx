      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {leagueId ? (
            selectedMember ? (
              <div className="flex items-center justify-between bg-white border border-slate-300 rounded px-3 py-2">
                <span>#{selectedMember.seed_number} {selectedMember.name}</span>
                <button type="button" onClick={() => setOpponentUserId(null)}
                  className="text-sm text-emerald-700">Change</button>
              </div>
            ) : playableMembers.length === 0 ? (
              <p className="text-sm text-slate-500">No active members in this league yet.</p>
            ) : (
              <ul className="bg-white border border-slate-200 rounded divide-y divide-slate-100 max-h-48 overflow-auto">
                {playableMembers.map((m) => (
                  <li key={m.id}>
                    <button type="button" onClick={() => setOpponentUserId(m.user_id)}
            ...
            <OpponentPicker contacts={contacts} value={opponentContactId} onChange={setOpponentContactId} />
          )}
        </div>
        ...
        <div className="flex gap-3">
          <label className="flex-1 text-sm">
            Your games
            <input type="number" min={0} max={3} value={yourGames} required
              onChange={(e) => setYourGames(Number(e.target.value))}
              className="mt-1 block w-full rounded border border-slate-300 px-3 py-2" />
          </label>
          <label className="flex-1 text-sm">
            Their games
            <input type="number" min={0} max={3} value={theirGames} required
        ...
        <button type="submit"
          disabled={saving || (leagueId ? !opponentUserId : !opponentContactId)}
